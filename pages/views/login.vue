      ...
      <div class="login-main-content">
        <div label-width="0" class="login-form">
          ...
          <div class="login-from-item">
            <input v-model="inputValue" :placeholder="TUITranslateService.t('请输入userID')" type="string" class="login-input-uniapp">
          </div>
          <div class="login-from-item">
            <div class="checked-text">
              <div class="private-switch" @click="onAgreePrivateProtocol">
                <image v-if="privateAgree" class="selected-icon icon-default" src="@/static/selected.svg" />
                <i v-else class="icon-unselected icon-default" />
              </div>
              <div class="private-content">
                <span>{{ TUITranslateService.t("我已阅读并同意") }}</span>
                <a class="private-content-link" @click="openFullPlatformLink(Link.privacy.url)">《{{ TUITranslateService.t(`${Link.privacy.label}`) }}》</a>{{ TUITranslateService.t("和") }}
                <a class="private-content-link" @click="openFullPlatformLink(Link.agreement.url)">《{{ TUITranslateService.t(`${Link.agreement.label}`) }}》</a>
              </div>
            </div>
          </div>
          <div class="login-btn">
