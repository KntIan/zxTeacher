    ...
    <div class="rating-card">
      ...
      <div class="card-wrapper">
        <div style="max-width: 200px">
          <div v-for="(item, index) in starList" :key="index" style="display: inline-block" @click="setValue(index)" @mouseenter="setHoverValue(index)" @mouseleave="setHoverValue(-1)">
      ...
      <button class="submit-button" :disabled="hasReply || hasExpire" @click="submitRatingStar">
        提交评价
      </button>
    </div>
    <p v-if="hasReply" class="rating-tail" :style="{
        marginTop: 20 + 'px',
      }">
    ...
    watchEffect(() => {
      ...
      if (props.ratingTemplate.selected != undefined) {
        for (let i = 0; i < props.ratingTemplate.menu.length; i++) {
          if (
            props.ratingTemplate.menu[i].id == props.ratingTemplate.selected.id
          ) {
            hasReply.value = true;
            value.value = i;
            break;
