<template>
	<div class="message-plugin">
		...
		<div v-if="props.showStyle === 'tip'" class="message-plugin-tip">
			<slot name="messageTip" />
		</div>
		<!-- The following is displayed in the form of messageBubble -->
		<div v-else-if="props.showStyle === 'bubble'" class="message-plugin-bubble-content"
			...
			<MessageBubble :messageItem="messageModel" :content="messageModel.getMessageContent()"
				:blinkMessageIDList="props.blinkMessageIDList" :classNameList="props.bubbleClassNameList"
				...
				<template #messageElement>
					<slot v-if="!isUniFrameWork" name="messageBubble" />
				</template>
