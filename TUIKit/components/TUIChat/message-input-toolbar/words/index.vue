<template>
	<ToolbarItemContainer ref="container" :iconFile="wordsIcon" title="常用语" :needBottomPopup="true"
		...
		<div :class="['words', !isPC && 'words-h5']">
			<div :class="['words-header', !isPC && 'words-h5-header']">
				...
				<span v-if="!isPC" :class="['words-header-close', !isPC && 'words-h5-header-close']"
					@click="closeDialog">
					关闭
				</span>
			</div>
			<ul :class="['words-list', !isPC && 'words-h5-list']">
				<li v-for="(item, index) in wordsList" :key="index"
					:class="['words-list-item', !isPC && 'words-h5-list-item']" @click="selectWord(item)">
