      ...
      'message-input-toolbar-uni',
		...
		<div v-else>
            ...
            'message-input-toolbar-uni-list',
					...
					<template v-if="currentExtensionList.length > 0">
						<div v-for="(extension, index) in currentExtensionList.slice(0, slicePos)" :key="index">
							<ToolbarItemContainer v-if="extension" :iconFile="genExtensionIcon(extension)"
								:title="genExtensionText(extension)" iconWidth="25px" iconHeight="25px"
								:needDialog="false" @onIconClick="onExtensionClick(extension)" />
						</div>
					</template>
					<template v-if="neededCountFirstPage === 1">
						<Evaluate v-if="featureConfig.InputEvaluation" @onDialogPopupShowOrHide="handleSwiperDotShow" />
						<Words v-else-if="featureConfig.InputQuickReplies"
				...
				<swiper-item v-if="neededCountFirstPage <= 1" :class="[
            'message-input-toolbar-list',
            'message-input-toolbar-h5-list',
            'message-input-toolbar-uni-list',
          ]">
