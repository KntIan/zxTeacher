<template>
	<ToolbarItemContainer ref="container" :iconFile="evaluateIcon" title="评价" :needBottomPopup="true"
		...
		<div :class="['evaluate', !isPC && 'evaluate-h5']">
            ...
            !isPC && 'evaluate-h5-content-list',
					...
					<li v-for="(item, index) in starList" :key="index" :class="[
              'evaluate-content-list-item',
              !isPC && 'evaluate-h5-content-list-item',
            ]" @click.stop="selectStar(index)">
						<Icon v-if="index <= currentStarIndex" :file="starLightIcon" :width="isPC ? '20px' : '30px'"
							:height="isPC ? '20px' : '30px'" />
						<Icon v-else :file="starIcon" :width="isPC ? '20px' : '30px'"
							:height="isPC ? '20px' : '30px'" />
					</li>
				</ul>
