<template>
	<div v-if="showDialog" class="dialog" :class="[!isPC ? 'dialog-h5' : '', center ? 'center' : '']"
		@click.stop.prevent="toggleView(clickType.OUTSIDE)">
		<main class="dialog-main" :class="[!backgroundDialog ? 'dialog-main-back' : '']"
			@click.stop.prevent="toggleView(clickType.INSIDE)">
			<header v-if="isHeaderShowDialog" class="dialog-main-header">
			...
			<footer v-if="isFooterShowDialog" class="dialog-main-footer">
