<template>
	<table :class="['tui-date-table', !isPC && 'tui-date-table-h5']" cellspacing="0" cellpadding="0" role="grid">
		<tbody class="tui-date-table-body">
			<tr class="tui-date-table-body-weeks">
				<th v-for="item in WEEKS" :key="item" class="tui-date-table-body-weeks-item" :aria-label="item + ''"
					scope="col">
					{{ TUITranslateService.t(`time.${item}`) }}
				</th>
			</tr>
			<tr v-for="(row, rowKey) in rows" :key="rowKey" class="tui-date-table-body-days">
				<td v-for="(col, colKey) in row" :key="colKey" :class="['tui-date-table-body-days-item', col.type]">
					<div :class="[
