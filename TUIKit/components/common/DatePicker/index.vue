<template>
	<div :class="[n([''])]">
		<div :class="[n(['input']), isDatePanelShow && n(['input-active'])]"
			@click="setDatePanelDisplay(!isDatePanelShow)">
			<slot name="start-icon" />
			<input v-model="startFormatDate" :placeholder="startPlaceholderVal" :class="[n(['input-start'])]"
				style="pointer-events: none" type="text" :readonly="true" :disabled="isUniFrameWork"
				autocomplete="false">
			<span v-if="type !== 'single'">-</span>
			<input v-if="type !== 'single'" v-model="endFormatDate" :placeholder="endPlaceholderVal"
				:class="[n(['input-end'])]" style="pointer-events: none" type="text" :readonly="true"
				:disabled="isUniFrameWork" autocomplete="false">
			<slot name="end-icon" />
		</div>
		<div v-if="isDatePanelShow" :class="[n(['dialog'])]">
			<div :class="[
          n([
            'dialog-container',
            ...
            'dialog-container-' + popupPosition,
				...
				<DatePickerPanel :type="props.type" rangeType="left" :date="dateValue" :startDate="startValue"
					:endDate="endValue" :currentOtherPanelValue="rightCurrentPanelValue" @pick="handlePick"
					@change="handleLeftPanelChange" />
				<DatePickerPanel v-if="props.type === 'range' && isPC && rangeTableType === 'two'" :type="props.type"
					rangeType="right" :date="dateValue" :startDate="startValue" :endDate="endValue"
					:currentOtherPanelValue="leftCurrentPanelValue" @pick="handlePick"
