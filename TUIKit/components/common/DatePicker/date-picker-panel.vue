		...
		<div :class="[n('body')]">
			<div :class="[n('body-header')]">
				<div :class="[n('body-header-prev')]">
					<div v-if="canYearLess" :class="[n('icon')]" @click="change('year', -1)">
						<Icon :file="dLeftArrowIcon" :width="'12px'" :height="'12px'" />
					</div>
					<div v-if="canMonthLess" :class="[n('icon')]" @click="change('month', -1)">
				...
				<div :class="[n('body-header-next')]">
					<div v-if="canMonthMore" :class="[n('icon')]" @click="change('month', 1)">
						<Icon :file="rightArrowIcon" :width="'10px'" :height="'10px'" />
					</div>
					<div v-if="canYearMore" :class="[n('icon')]" @click="change('year', 1)">
						<Icon :file="dRightArrowIcon" :width="'12px'" :height="'12px'" />
					</div>
				</div>
			</div>
			<div :class="[n('body-content')]">
				<DateTable :type="props.type" :date="props.date" :startDate="props.startDate" :endDate="props.endDate"
					:currentPanelDate="currentPanelDate" @pick="handlePick" />
			</div>
		</div>
	</div>
</template>
