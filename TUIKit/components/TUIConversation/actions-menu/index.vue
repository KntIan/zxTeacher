<template>
	<Overlay :maskColor="'transparent'" @onOverlayClick="() => emits('closeConversationActionMenu')">
		<div id="conversation-actions-menu" ref="actionsMenuDomRef" :class="[
        ...
        left: `${_actionsMenuPosition.left}px`,
			...
			<div v-if="!(props.selectedConversation && props.selectedConversation.isPinned)"
				:class="['actions-menu-item']" @click.stop="handleItem({ name: CONV_OPERATION.ISPINNED })">
				{{ TUITranslateService.t("TUIConversation.置顶会话") }}
			</div>
			<div v-if="props.selectedConversation && props.selectedConversation.isPinned" :class="['actions-menu-item']"
				@click.stop="handleItem({ name: CONV_OPERATION.DISPINNED })">
				{{ TUITranslateService.t("TUIConversation.取消置顶") }}
			</div>
			<div v-if="!(props.selectedConversation && props.selectedConversation.isMuted)"
				:class="['actions-menu-item']" @click.stop="handleItem({ name: CONV_OPERATION.MUTE })">
				{{ TUITranslateService.t("TUIConversation.消息免打扰") }}
			</div>
			<div v-if="props.selectedConversation && props.selectedConversation.isMuted" :class="['actions-menu-item']"
