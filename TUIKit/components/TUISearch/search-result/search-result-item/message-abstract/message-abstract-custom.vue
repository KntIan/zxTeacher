	...
	<div :class="['message-abstract-custom']" @click.capture.stop>
		<template v-if="businessID === CHAT_MSG_CUSTOM_TYPE.SERVICE">
			<div :class="['service']">
				...
				<ul v-if="extensionJSON.item && extensionJSON.item.length > 0" :class="['service-list']">
					<li v-for="(item, index) in extensionJSON.item" :key="index" :class="['service-list-item']">
						<a v-if="isUrl(item.value)" :class="['service-list-item-link', 'link']" :href="item.value"
		...
		<template v-else-if="businessID === CHAT_MSG_CUSTOM_TYPE.EVALUATE">
			<div class="evaluate">
				...
				<ul v-if="extensionJSON.score" class="evaluate-list">
					<li v-for="(item, index) in Math.max(extensionJSON.score, 0)" :key="index"
