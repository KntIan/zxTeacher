<template>
	...
	<div v-else :class="[
      ...
      isPC && isResultDetailShow && 'tui-search-result-with-border',
		...
		<div v-if="props.searchType !== 'conversation' && (isPC || !isResultDetailShow)" class="tui-search-result-main">
			<div class="tui-search-result-list">
				<div v-for="result in searchResult" :key="result.key" class="tui-search-result-list-item">
					<div v-if="props.searchType === 'global'" class="header">
						{{ TUITranslateService.t(`TUISearch.${result.label}`) }}
					</div>
					<div class="list">
						<div v-for="item in result.list" :key="item.conversation.conversationID"
							...
							<SearchResultItem
								...
								@showResultDetail="showResultDetail" @navigateToChatPosition="navigateToChatPosition" />
						</div>
					</div>
					<div v-if="currentSearchTabKey === 'all' || result.cursor" class="more"
						@click="getMoreResult(result)">
						<Icon class="more-icon" :file="searchIcon" width="12px" height="12px" />
						<div class="more-text">
		...
		<div v-if="isResultDetailShow || props.searchType === 'conversation'" :class="[
        ...
        props.searchType === 'conversation' && 'tui-search-result-in-conversation',
			...
			<div v-if="!isSearchDetailLoading && isResultDetailShow && props.searchType !== 'conversation'"
              ...
              TUITranslateService.t(
				...
				<div class="header-enter" @click="enterConversation({ conversationID: currentSearchConversationID })">
					...
					<Icon class="enter-icon" :file="enterIcon" width="14px" height="14px" />
				</div>
			</div>
			<div v-if="!isSearchDetailLoading &&
        ...
        " class="tui-search-message-list">
          ...
          ">
					<div v-for="item in searchConversationMessageList" :key="generateVueRenderKey(item.ID)"
						:class="['list-item']">
						<SearchResultItem :listItem="item" :listItemContent="item.getMessageContent()"
							:type="currentSearchTabKey" :displayType="generateResultItemDisplayType()"
							:keywordList="keywordList" @showResultDetail="showResultDetail"
				...
				<template v-else>
					<div v-for="group in searchConversationResultGroupByDate" :key="generateVueRenderKey(group.date)"
						:class="['list-group', currentSearchTabKey === 'fileMessage'? 'list-group-file' : 'list-group-image']">
						<div :class="['list-group-date']">
							{{ group.date }}
						</div>
						<div v-for="item in group.list" :key="generateVueRenderKey(item.ID)"
				...
				<div v-if="searchConversationResult && searchConversationResult.cursor" class="more"
